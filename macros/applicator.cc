...
void applicator(TString outDir, 
                TString inDir,
                TString inFileName,
                TString inTreeName,
                TString outTreeName,
...

{
  ...
  TTreeFormula *photonPtF = new TTreeFormula("PhotonPt",PhotonPtExpression,inTree);
  TTreeFormula *ZPtF      = new TTreeFormula("ZPt",DiLeptonPtExpression,inTree);
  TTreeFormula *genBosPtF      = new TTreeFormula("GenBosPt",GenBosonPtExpression,inTree);
  TTreeFormula *genBosPdgIdF   = new TTreeFormula("GenBosPdgId",GenBosonPdgIdExpression,inTree);
  TTreeFormula *daughterPdgIdF = new TTreeFormula("DaughterPdgId",DaughterPdgIdExpression,inTree);
  TTreeFormula *eventNumF      = new TTreeFormula("EventNum",EventNumExpression,inTree);
  ...
  for (int entry = 0; entry != numEntries; ++entry) {
    ...
    float photonPt = float(photonPtF->EvalInstance());
    float ZPt      = float(ZPtF->EvalInstance());

    float genBosPt    = float(genBosPtF->EvalInstance());
    int genBosPdgId   = int(genBosPdgIdF->EvalInstance());
    int daughterPdgId = int(daughterPdgIdF->EvalInstance());
    int eventNum      = int(eventNumF->EvalInstance());
